...
const form = useForm({
    key: props.givingType.key,
    name: props.givingType.name,
    description: props.givingType.description || '',
    contribution_type: props.givingType.contribution_type || 'individual',
});

function updateGivingType() {
    form.put(`/giving-types/${props.givingType.id}`);
}
</script>
...
<template>
    <AuthLayout>
        <template #page-title>
            <h1 class="m-0">Edit Giving Type</h1>
        </template>

        <template #page-content>
            <FluidContainerWithRow>
                <div class="col-12">
                    <Card :with-card-header="true" card-title="Edit Giving Type">
                        <template #card-body>
                            <div>
                            ...
                            <form method="post" :action="`/giving-types/${props.givingType.id}`" @submit.prevent="updateGivingType">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-6">
